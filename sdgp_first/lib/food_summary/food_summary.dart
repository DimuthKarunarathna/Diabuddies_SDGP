  ...
  Future<void> getFoodHistory() async {
    ...
    documents.forEach((document) {
      ...
      if(data!=null){
        ...
        double afbgl = data['afbgl'].toDouble();
        double calories=data['calories'].toDouble();

        // ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("working2")));
        entities.add(FoodEntity(time: time, date: date, carbs: carbs, protein: protein, fiber: fiber, fats: fats, calories: calories, bglBefore: bebgl, bglAfter: afbgl));
        setState(() {

        });
      }else{
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("working3")));
      }
